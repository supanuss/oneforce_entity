...
- deterministic code ใช้เฉพาะ validate, normalize รูปแบบเล็กน้อย, guardrail, คำนวณ `damage_amount`, สร้าง transfer/refund timeline, ให้คะแนนความน่าเชื่อถือ, และเขียน KG
...
`TimelineAgent` เป็น hybrid node สำหรับสร้าง Case Narrative Timeline

ทำงาน 2 ส่วน:

- deterministic parser สร้าง `TRANSFER`, `REFUND`, และ `REPORT` events จากข้อความดิบ
- LLM narrative agent สกัด `DISCOVERY`, `CONTACT`, `PERSUASION`, `REALIZATION`, `REPORT`, `LEGAL_ACTION` จากข้อความ narrative ที่ตัดรายการโอนออกแล้ว

เพื่อให้ qwen 9B รอด:

- ใช้ `TIMELINE_NARRATIVE_CONTEXT_MAX_CHARS=2500`
- ใช้ `LLM_TIMELINE_NUM_PREDICT=1024`
- จำกัด narrative events ไม่เกิน 6 รายการ
- ตัวเลข ยอดเงิน เวลาโอน และบัญชีปลายทางมาจาก deterministic parser ไม่ใช่ LLM
...
```json
{
  "event_order": 6,
  "event_type": "TRANSFER",
  "event_time": "29 มิถุนายน 2569 14:57",
  "amount": 890.0,
  "destination_account": "1808240226",
  "description": "โอนเงิน 890.00 บาท ไปยังบัญชี 1808240226",
...
```python
case = state["cases"][state["current_index"]]
extracted = state["temp_extraction"]
```

ถ้า `KG_ENABLED=true` จะเขียน Neo4j แบบ idempotent ด้วย `MERGE`

ถ้า Neo4j ไม่พร้อม จะ mark:
...
แล้ว workflow ยัง Save extraction ต่อได้ ไม่ทำให้ข้อมูลสกัดหาย
...
- ตรวจว่า transfer timeline รวมยอดตรงกับ `damage_amount`
- ตรวจว่าบัญชีปลายทางใน `TransferEvent` อยู่ใน `bank_accounts`
- ถ้าผิด ส่ง feedback กลับไป retry เฉพาะ agent ที่เกี่ยวข้อง
- ระบุ `retry_target` เป็น `entity`, `profile`, `relation`, หรือ `timeline`
...
```json
{
  ...
  "confidence_score": {},
  "kg_status": "written | failed | disabled",
  "case_id": "...",
  "accuser_original": "..."
}
```
...
ระหว่างรันแต่ละเคส ระบบจะให้คะแนนแบบ provisional ก่อน จากนั้นหลังจบ batch จะ rescore ทั้งชุดอีกครั้ง โดยอ่าน historical threat index จาก SQLite แล้ว merge กับเคสใน batch ปัจจุบัน เพื่อให้ duplicate account/contact ตรวจได้ทั้งจากประวัติเดิมและข้อมูลรอบล่าสุด
...
Threat intelligence index ถูกเก็บแบบ historical SQLite ที่:
...
แต่ละตารางเก็บ `first_seen`, `last_seen`, และ `source` ด้วย และใช้ unique pair:

- `(account_number, case_id)`
- `(contact_key, case_id)`

ดังนั้นการรันซ้ำเคสเดิมจะไม่เพิ่มแถวซ้ำ แต่จะอัปเดต `last_seen` แทน
...
ทำให้ analyst เปิดกราฟแล้วเข้าใจคดีเร็ว:
...
(:Case)-[:INVOLVES_SCAM_TYPE]->(:ScamType)
(:Case)-[:STARTED_FROM]->(:HookPoint)
(:Case)-[:USED_TACTIC]->(:PsychologicalTactic)
(:Case)-[:CONTACTED_VIA]->(:ContactChannel)
```

### 2. Timeline/Event Layer

ใช้ดูว่าโดนหลอกยังไง แล้วเงินไหลไปไหนตามลำดับ:

```text
(:Case)-[:HAS_EVENT]->(:Event)
(:Event)-[:NEXT_EVENT]->(:Event)
(:Event)-[:TRANSFER_TO]->(:BankAccount)
```
...
### 3. Intelligence/Assertion Layer

ใช้ขยายผลเชิง forensic และรองรับ LLM relations โดยไม่ทำให้ canonical graph สกปรก:

```text
(:Case)-[:HAS_ASSERTION]->(:Assertion)
(:Assertion)-[:ASSERTS_SUBJECT]->(:Entity)
(:Assertion)-[:ASSERTS_OBJECT]->(:Entity)
```
...
ถ้า subject/object จาก LLM relation resolve ไม่เจอ canonical entity ระบบจะสร้าง `(:EvidenceEntity)` แทน
...
- duplicate account/contact ใน historical SQLite ปัจจุบัน: ไม่พบ
...
LLM_RELATION_NUM_PREDICT=2048
LLM_TIMELINE_NUM_PREDICT=1024
RELATION_CONTEXT_MAX_CHARS=3500
TIMELINE_NARRATIVE_CONTEXT_MAX_CHARS=2500
LLM_TIMEOUT=240
LLM_REASONING=false
CASE_LIMIT=10
MAX_RETRY_LIMIT=3
...
5. System calculates damage_amount and account transfer_amount deterministically
...
timeline error -> TimelineAgent -> Guardrail
manual_review  -> Save
pass           -> Save
```
